...
def run_stock_job():
    ...
    try:
        ...
        print(f"Error while fetching data from Polygon API: {e}")
        # Even if error, save what we have so far
        if tickers:
            print("Saving partial data to Snowflake after error...")
            load_to_snowflake(tickers, fieldnames)
        raise  # Optionally re-raise, or just return

    # Save whatever we've fetched (whether loop completes or not)
    load_to_snowflake(tickers, fieldnames)
